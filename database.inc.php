...
{	global $DatabaseServer,$DatabaseUsername,$DatabasePassword,$DatabaseName,$DatabasePort,$DatabaseType;
	...
	if($connection === false)
	{
		...
		db_show_error("","$databaseinc1: $DatabaseServer",$errstring);
	}
	return $connection;
}
...
{	global $DatabaseType,$_openSIS;
	...
	switch($DatabaseType)
	{
		case 'mysql':
				...
				foreach( $array as $value )
				{
					if($value!="")
					{
						...
						if(!$result)
						{
							...
							die(db_show_error($sql,'$databaseinc2',mysql_error()));
						}
					}
				}
			}
			...
			{
				$result = mysql_query($sql) or die(db_show_error($sql,$databaseinc2,mysql_error()));
			}
		break;
	}
	return $result;
